  ...
  io.on('connection', async (socket: AuthenticatedSocket) => {
    ...
    socket.on(SOCKET_EVENTS.TYPING_START, ({ conversationId }: TypingPayload) => {
      safeAsync(async () => {
        ...
        socket
          .to(conversationId)
          .emit(SOCKET_EVENTS.TYPING_START, { conversationId, userName: username });
        ...
        const timer = setTimeout(() => {
          socket
            .to(conversationId)
            .emit(SOCKET_EVENTS.TYPING_STOP, { conversationId, userName: username });
