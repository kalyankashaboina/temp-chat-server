...
export const limitSchema = (max: number = PAGINATION.MAX_LIMIT, def: number = PAGINATION.DEFAULT_LIMIT) =>
...
export const updateProfileSchema = z
  .object({
    ...
    bio:    z.string().max(LIMITS.BIO_MAX, `Bio must be at most ${LIMITS.BIO_MAX} characters`).optional(),
  })
...
export const attachmentSchema = z.object({
  ...
  mimeType: z.string().min(1).refine(
    (v) => UPLOAD.ALLOWED_MIMES.includes(v),
